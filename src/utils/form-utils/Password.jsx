            ...
            <div className="relative">
                <Field id={name} name={name} {...rest}>
                    {({ field, form: { setFieldValue } }) => {
                        const { value } = field
                        return <input {...rest} type={`${isOpen ? "text" : "password"}`} {...field} value={value} onChange={e => setFieldValue(name, e.target.value)} />
                    }}
                </Field>
                <div className='absolute right-1 px-1  top-[50%] -translate-y-[50%] h-[90%] flex justify-center items-center'>
